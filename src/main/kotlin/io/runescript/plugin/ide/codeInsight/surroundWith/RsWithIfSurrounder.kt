...
class RsWithIfSurrounder : Surrounder {
    ...
    override fun surroundElements(
        ...
        val template = RsElementGenerator.createStatement(project, "if (true) {\n$source\n}")
        val inserted = statementList.addBefore(template, first) as RsIfStatement
        statementList.deleteChildRange(first, last)
        val formatted = CodeStyleManager.getInstance(project).reformat(inserted) as RsIfStatement

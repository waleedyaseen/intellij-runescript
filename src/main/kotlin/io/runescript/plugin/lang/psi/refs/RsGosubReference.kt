...
class RsGosubReference(element: RsGosubExpression) : PsiPolyVariantReferenceBase<RsGosubExpression>(element, element.nameLiteral.textRangeInParent) {
    ...
    override fun multiResolve(incompleteCode: Boolean): Array<ResolveResult> {
        val elements = StubIndex.getElements(RsGotoScriptIndex.KEY, element.nameLiteral.text, element.project, GlobalSearchScope.allScope(element.project), RsScriptName::class.java)
                .filter { it.triggerExpression.text == "proc" }
